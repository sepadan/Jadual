...
test("a visitor cannot export or print the relief sheet", () => {
  // Export and print belong to the office, not to the teacher reading the public page: the toolbar
  // row carries admin-only, which the public-mode rule hides.
  assert.match(html, /<div class="button-row admin-only"><button id="exportReliefPdf"/, "the relief export row is not admin-only");
  assert.match(html, /\.public-mode \.admin-only\{display:none!important\}/, "the public-mode rule that hides admin controls is gone");
});
