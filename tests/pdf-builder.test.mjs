...
test('reviewed aSc import populates every editable builder section',()=>{
  ...
  assert.equal(state.kelas[0].nama,'4 BIJAK');assert.equal(state.kelas[0].guruKelas,state.guru[0].id);
  assert.equal(state.subjek[0].kod,'BM');assert.equal(state.agihan[0].waktu,2);assert.equal(state.peruntukan[state.subjek[0].id][4],2);
  ...
  assert.equal(state.acara[0].kod,'KOKU');assert.equal(state.acara[0].skop,'semua');assert.equal(state.masa.mula,'07:30');
});
